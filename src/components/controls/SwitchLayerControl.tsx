import { AVAILABLE_LAYERS } from 'constants/mapLayers'
import { Map } from 'ol'
import { Control } from 'ol/control'
import { useEffect, useRef } from 'react'

export interface SwitchLayerControlProps {
    mapRef: React.MutableRefObject<Map | undefined> 
    activeLayer: string,
    setActiveLayer: (layer: string) => void,
}

const SwitchLayerControl = (props: SwitchLayerControlProps) => {
    ...
    const handlerSwitchView = () => {
        if (!props.mapRef.current) return
        const layers = props.mapRef.current.getLayers();
        const keys = Object.keys(AVAILABLE_LAYERS);
        const newActiveLayer = keys[(keys.indexOf(props.activeLayer) + 1) % keys.length];
        props.setActiveLayer(newActiveLayer);
